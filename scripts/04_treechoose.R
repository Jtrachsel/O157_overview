...
download.file('https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/013/168/095/GCF_013168095.1_ASM1316809v1/GCF_013168095.1_ASM1316809v1_genomic.fna.gz', 
              destfile='data/O157/86_24.fna.gz')
...
# which ones are missed and why!?!?!

TMP <- sub('./data/O157/(.*).fna','\\1',incomplete_genomes)

# these are not in the O157 serotyped directory
# they are missing the O antigen
MISSING_REPS <- 
  tree_data_small %>%
  filter(!(representative %in% TMP)) %>% 
  transmute(asm_acc=representative, num_genomes) %>% 
  left_join(meta)
...
usethis::use_directory('data/dan_lin/')

dan_lin_copy <- 
  tibble(PATH=c(complete_genomes, incomplete_genomes)) %>% 
  mutate(NEW_PATH=sub('./data/O157/(.*)','./data/dan_lin/\\1',PATH))
